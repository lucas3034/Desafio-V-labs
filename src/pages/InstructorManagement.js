...
import { courseService, userService, externalService } from '../services/api';
import { Card, Button, Grid, Flex, LoadingSpinner } from '../styles/GlobalStyles';
...
const RoleBadge = styled.span`
  background: ${props => props.isCreator ? props.theme.colors.primary : props.theme.colors.success};
...
const InstructorManagement = () => {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const courseInstructors = usersData.filter(u => 
        courseData.instructors.includes(u.id)
      );
  ...
  const loadExternalUsers = async () => {
    ...
    try {
      const users = await externalService.getRandomUsers(8);
  ...
  const removeInstructor = async (instructorId) => {
    ...
    try {
      const updatedInstructors = course.instructors.filter(id => id !== instructorId);
  ...
  const addExternalInstructor = async (externalUser) => {
    try {
      const existingUser = instructors.find(instructor => 
        instructor.email === externalUser.email
      );
      ...
      setExternalUsers(prev => prev.filter(u => u.email !== externalUser.email));
            ...
            {instructors.map(instructor => (
              <InstructorCard key={instructor.id}>
                <RoleBadge isCreator={instructor.id === course.creator_id}>
                  {instructor.id === course.creator_id ? 'Criador' : 'Instrutor'}
                </RoleBadge>
          ...
          <Grid>
            {externalUsers.map((externalUser, index) => (
              <ExternalUserCard key={index}>
                <UserAvatar src={externalUser.picture} alt={externalUser.name} />
