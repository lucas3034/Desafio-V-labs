...
import { Card, Button, Input, Select, Grid, Flex, LoadingSpinner, Badge } from '../styles/GlobalStyles';
...
const PageHeader = styled.div`
  ...
  margin: -${props => props.theme.spacing.xl} -${props => props.theme.spacing.xl} ${props => props.theme.spacing.xl};
  border-radius: 0 0 ${props => props.theme.borderRadius.xxl} ${props => props.theme.borderRadius.xxl};
  position: relative;
  overflow: hidden;
...
const FiltersCard = styled(Card)`
  margin-bottom: ${props => props.theme.spacing.xl};
  background: linear-gradient(135deg, ${props => props.theme.colors.white} 0%, ${props => props.theme.colors.accent}05 100%);
`;
...
const CourseDetails = () => {
  ...
  const isInstructor = () => {
    ...
    const isInstructorResult = course.instructors.some(instructorId => {
    ...
    <div>
      <PageHeader>
        <HeaderContent>
          ...
          <QuickStats>
            ...
            <StatItem>
              <div className="number">{course.instructors.length}</div>
          ...
          <ActionButtons>
            {isInstructor() && (
              <Button as={Link} to={`/courses/${id}/lessons/new`} variant="secondary" size="large">
                ➕ Nova Aula
              </Button>
            )}
            {isCreator() && (
              <>
                <Button as={Link} to={`/courses/${id}/instructors`} variant="secondary" size="large">
                  👥 Gerenciar Instrutores
                </Button>
                <Button as={Link} to={`/courses/${id}/edit`} variant="secondary" size="large">
                  ✏️ Editar Curso
                </Button>
                <Button onClick={handleDeleteCourse} variant="danger" size="large">
                  🗑️ Excluir Curso
                </Button>
              </>
            )}
      ...
      <LessonsSection>
        <SectionHeader>
          ...
          {isInstructor() && (
            <Button as={Link} to={`/courses/${id}/lessons/new`} variant="gradient">
              ➕ Adicionar Aula
            </Button>
          )}
        </SectionHeader>
        ...
        {filteredLessons.length === 0 ? (
          <EmptyState>
            ...
            {isInstructor() && lessons.length === 0 && (
              <Button as={Link} to={`/courses/${id}/lessons/new`} variant="gradient" size="large">
                🚀 Criar Primeira Aula
              </Button>
            )}
          </EmptyState>
                ...
                return (
                  ...
                  >
                    <LessonHeader>
                      ...
                      >
                        {thumbnailUrl ? (
                          <img src={thumbnailUrl} alt={`Thumbnail for ${lesson.title}`} />
                        ) : (
                          '📹'
                        )}
                      </VideoThumbnail>
